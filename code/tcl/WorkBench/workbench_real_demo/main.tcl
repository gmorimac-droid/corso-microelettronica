...
namespace eval wb {
    ...
    variable externalExecActive 0
    variable externalExecChan ""
    variable externalExecNode ""
    variable externalExecCmd {}
    variable externalExecStdoutPath ""
    ...
    variable launchProfiles {
        {
            ...
            type "python_script"
            launcher "python3"
            target "tools/demo_runner.py"
            extra_args "--mode radar --emit-report"
        }
        {
...
proc wb::newProjectDialog {} {
    ...
    catch {wb::createDemoExternalTools}
    wb::refreshAll
...
# SAMPLE SEED
# ==================================================
proc wb::createDemoExternalTools {} {
    set root [wb::projectRoot]
    set toolsDir [file join $root tools]
    wb::ensureDir $toolsDir
    set py [file join $toolsDir demo_runner.py]
    if {![file exists $py]} {
        set fh [open $py w]
        puts $fh {import argparse, csv, os, time}
        puts $fh {p = argparse.ArgumentParser()}
        puts $fh {p.add_argument('--config', required=True)}
        puts $fh {p.add_argument('--output', required=True)}
        puts $fh {p.add_argument('--mode', default='radar')}
        puts $fh {p.add_argument('--emit-report', action='store_true')}
        puts $fh {p.add_argument('--waves', action='store_true')}
        puts $fh {args = p.parse_args()}
        puts $fh {os.makedirs(args.output, exist_ok=True)}
        puts $fh {print(f'INFO workflow external runner mode={args.mode}')}
        puts $fh {print(f'INFO reading config={args.config}')}
        puts $fh {time.sleep(0.2)}
        puts $fh {print('OK preprocessing completed')}
        puts $fh {time.sleep(0.2)}
        puts $fh {with open(os.path.join(args.output, 'external_metrics.csv'), 'w', newline='') as f:}
        puts $fh {    w = csv.writer(f); w.writerow(['metric','value','min','max']); w.writerow(['latency_ms', 9 if args.mode=='hdl' else 11, 0, 10]); w.writerow(['snr_db', 24, 18, 40]); w.writerow(['tracks', 41, 40, 999]); w.writerow(['false_alarm_rate', 0.6, 0, 1.0])}
        puts $fh {with open(os.path.join(args.output, 'external_report.txt'), 'w') as f: f.write(f'completed mode={args.mode}
config={args.config}
')}
        puts $fh {print('OK external artifacts written')}
        puts $fh {if args.waves: open(os.path.join(args.output, 'waves.vcd'), 'w').write('$date
 demo 
$end
')}
        puts $fh {print('COMPLETED external runner')}
        close $fh
    }
}
...
proc wb::resolveProfileCommandForOutput {outputDir} {
    set p [wb::findProfileByName $::wb::launchProfile]
    if {$p eq ""} {
        return [dict create ok 0 message "launcher profile non trovato" cmd {}]
    }
    ...
    set target [dict get $p target]
    set pextra [dict get $p extra_args]
    set cfg $::wb::launchConfigPath
    set root [wb::projectRoot]

    if {$cfg ne "" && [file pathtype $cfg] ne "absolute"} {
        set cfg [file join $root $cfg]
    }
    if {$target ne "" && [file pathtype $target] ne "absolute"} {
        set maybe [file join $root $target]
    ...
    switch -- $ptype {
        python_script {
            set exe [auto_execok $launcher]
            if {$exe eq ""} { return [dict create ok 0 message "interprete python non trovato: $launcher" cmd {}] }
            if {$target eq "" || ![file exists $target]} { return [dict create ok 0 message "script python non trovato: $target" cmd {}] }
            lappend cmd $exe $target --config $cfg --output $outputDir
            foreach a [wb::splitArgs $pextra] { lappend cmd $a }
            foreach a [wb::splitArgs $::wb::launchExtraArgs] { lappend cmd $a }
        }
        cpp_executable {
            set exe $launcher
            if {[file pathtype $exe] ne "absolute"} {
                set maybe [file join $root $exe]
                if {[file exists $maybe]} { set exe $maybe }
            }
            if {[auto_execok $exe] eq "" && ![file executable $exe]} { return [dict create ok 0 message "eseguibile non trovato: $exe" cmd {}] }
            lappend cmd $exe --config $cfg --output $outputDir
            foreach a [wb::splitArgs $pextra] { lappend cmd $a }
            foreach a [wb::splitArgs $::wb::launchExtraArgs] { lappend cmd $a }
        }
        hdl_runner {
            set exe [auto_execok $launcher]
            if {$exe eq ""} { return [dict create ok 0 message "runner HDL non trovato: $launcher" cmd {}] }
            if {$target eq "" || ![file exists $target]} { return [dict create ok 0 message "script HDL non trovato: $target" cmd {}] }
            lappend cmd $exe $target --config $cfg --output $outputDir
            foreach a [wb::splitArgs $pextra] { lappend cmd $a }
            foreach a [wb::splitArgs $::wb::launchExtraArgs] { lappend cmd $a }
        }
        matlab_function {
            set exe [auto_execok $launcher]
            if {$exe eq ""} { return [dict create ok 0 message "matlab non trovato: $launcher" cmd {}] }
            set expr [format "%s('%s')" $target $cfg]
            lappend cmd $exe -batch $expr
        }
        default {
            set exe [auto_execok $launcher]
            if {$exe eq ""} { set exe $launcher }
            lappend cmd $exe
            foreach a [wb::splitArgs $::wb::launchExtraArgs] { lappend cmd $a }
        }
    }
    return [dict create ok 1 message "" cmd $cmd]
}

proc wb::startExternalNodeProcess {node cmd stdoutPath} {
    set runId $::wb::workflowExecRunId
    set nodeId [wb::safeDictGet $node nodeId]

    set ::wb::externalExecNode $node
    set ::wb::externalExecCmd $cmd
    set ::wb::externalExecStdoutPath $stdoutPath
    set ::wb::externalExecActive 1

    wb::appendFileLine $stdoutPath "[wb::nowString] COMMAND [join $cmd { }]"
    wb::logRuntime "INFO: launching external command node=$nodeId"

    if {[catch {open [concat | $cmd {2>@1}] r} ch err]} {
        set ::wb::externalExecActive 0
        wb::appendFileLine $stdoutPath "[wb::nowString] ERROR open failed: $err"
        wb::workflowNodeDone $node fail "launch failed: $err"
        return
    }

    set ::wb::externalExecChan $ch
    fconfigure $ch -blocking 0 -buffering line -translation auto
    fileevent $ch readable wb::onExternalNodeReadable
}

proc wb::onExternalNodeReadable {} {
    if {!$::wb::externalExecActive || $::wb::externalExecChan eq ""} { return }
    set ch $::wb::externalExecChan
    set node $::wb::externalExecNode
    set stdoutPath $::wb::externalExecStdoutPath

    while {[gets $ch line] >= 0} {
        wb::appendFileLine $stdoutPath $line
        wb::logRuntime $line
    }

    if {[eof $ch]} {
        set status ok
        set details "external command completed"
        if {[catch {close $ch} err]} {
            set status fail
            set details "external command failed: $err"
            wb::appendFileLine $stdoutPath "[wb::nowString] ERROR close failed: $err"
        }
        set ::wb::externalExecChan ""
        set ::wb::externalExecActive 0
        wb::workflowNodeDone $node $status $details
    }
}
...
proc wb::stopRun {} {
    ...
    if {$::wb::externalExecChan ne ""} { catch {close $::wb::externalExecChan}; set ::wb::externalExecChan "" }
    set ::wb::externalExecActive 0
...
proc wb::workflowNode_run {node} {
    ...
    set resolved [wb::resolveProfileCommandForOutput $outdir]
    if {![dict get $resolved ok]} {
        wb::appendFileLine $stdoutPath "[wb::nowString] ERROR [dict get $resolved message]"
        project::register_artifact             -runId $runId             -experimentId [wb::runField $runId experimentId]             -testcaseId [wb::runField $runId testcaseId]             -workflowId [wb::runField $runId workflowId]             -nodeId $nodeId             -path $stdoutPath             -label "Node stdout $nodeId"             -producer workflow
        after 50 [list wb::workflowNodeDone $node fail [dict get $resolved message]]
        return
    }

    project::register_artifact         -runId $runId         -experimentId [wb::runField $runId experimentId]         -testcaseId [wb::runField $runId testcaseId]         -workflowId [wb::runField $runId workflowId]         -nodeId $nodeId         -path $stdoutPath         -label "Node stdout $nodeId"         -producer workflow

    wb::startExternalNodeProcess $node [dict get $resolved cmd] $stdoutPath
}
