...
proc wb::loadPreset {domain} {
    ...
    wb::log "Preset caricato: $domain"
}

# -----------------------------
# Logging colorato
# -----------------------------
proc wb::configureLogTags {} {
    variable ui
    $ui(log) tag configure error -foreground red
    $ui(log) tag configure warn  -foreground darkorange3
    $ui(log) tag configure ok    -foreground darkgreen
    $ui(log) tag configure info  -foreground black
}
...
proc wb::log {msg} {
    ...
    if {[string match "ERROR:*" $msg]} {
        set tag error
    } elseif {[string match "WARN:*" $msg]} {
        set tag warn
    } elseif {[string match "OK:*" $msg]} {
        set tag ok
    }
...
proc wb::jsonParseString {text idxVar} {
    ...
    while {$i < $n} {
        ...
        if {$ch eq "\\"} {
            incr i
            if {$i >= $n} { error "Escape JSON incompleto" }
...
proc wb::jsonParseArray {text idxVar} {
    ...
    if {[string index $text $i] ne "["} {
        error "Array JSON atteso alla posizione $i"
    }
    incr i
    ...
    while 1 {
        ...
        if {$ch eq ","} {
            ...
            error "Separatore array JSON non valido alla posizione $i"
        }
    }
    return $result
}
...
proc wb::jsonParseObject {text idxVar} {
    ...
    if {[string index $text $i] ne "\{"} {
        error "Oggetto JSON atteso alla posizione $i"
    }
    incr i
    wb::jsonSkipWs $text i
    ...
    while 1 {
        ...
        if {$ch eq ","} {
            ...
            error "Separatore oggetto JSON non valido alla posizione $i"
        }
    }
...
proc wb::jsonParseValue {text idxVar} {
    ...
    switch -- $ch {
        ...
        default {
            ...
            error "Valore JSON non riconosciuto alla posizione $i"
        }
    }
}
...
proc wb::saveAppState {} {
    ...
    if {[catch {
        ...
        wb::log "WARN: impossibile salvare lo stato applicazione: $err"
    }
}
...
proc wb::loadAppState {} {
    ...
    if {[catch {
        ...
        wb::log "WARN: impossibile leggere lo stato applicazione: $err"
    }
}

# -----------------------------
# Config editor model <-> UI
# -----------------------------
...
proc wb::loadConfigToUI {cfgDict} {
    ...
    if {![dict exists $cfgDict domain]} {
        error "Campo 'domain' mancante nella configurazione"
    }
    ...
    foreach k {duration_s fs_hz target_count environment step_s orbit_type timescale seed} {
        if {[dict exists $cfgDict scenario $k]} {
            wb::setCfgField scenario.$k [dict get $cfgDict scenario $k]
        } else {
            wb::setCfgField scenario.$k ""
        }
    }
    ...
    foreach k {
        ...
        if {[dict exists $cfgDict sensor $k]} {
            wb::setCfgField sensor.$k [dict get $cfgDict sensor $k]
        } else {
            wb::setCfgField sensor.$k ""
        }
    }
    ...
    foreach k {
        ...
        if {[dict exists $cfgDict processing $k]} {
            wb::setCfgField processing.$k [dict get $cfgDict processing $k]
        } else {
            wb::setCfgField processing.$k ""
        }
    }
...
proc wb::collectConfigFromUI {} {
    ...
    switch -- $currentDomain {
        radar {
            foreach k {duration_s fs_hz target_count environment} {
                dict set cfgDict scenario $k [wb::getCfgField scenario.$k]
            }
            foreach k {fc_hz bandwidth_hz tx_power_w gain_db} {
                dict set cfgDict sensor $k [wb::getCfgField sensor.$k]
            }
            foreach k {threshold_mode integration_pulses tracker} {
                dict set cfgDict processing $k [wb::getCfgField processing.$k]
            }
        }
        satellite {
            foreach k {duration_s step_s orbit_type environment} {
                dict set cfgDict scenario $k [wb::getCfgField scenario.$k]
            }
            foreach k {fov_deg altitude_m gain_db pointing_mode} {
                dict set cfgDict sensor $k [wb::getCfgField sensor.$k]
            }
            foreach k {coverage_mode export_access_windows estimator} {
                dict set cfgDict processing $k [wb::getCfgField processing.$k]
            }
        }
        underwater {
            ...
            foreach k {center_freq_hz array_elements gain_db platform_speed_ms} {
                dict set cfgDict sensor $k [wb::getCfgField sensor.$k]
            }
            foreach k {beamforming detector threshold_mode} {
                dict set cfgDict processing $k [wb::getCfgField processing.$k]
            }
        }
        hdl {
            foreach k {timescale seed environment} {
                dict set cfgDict scenario $k [wb::getCfgField scenario.$k]
            }
            foreach k {dut_name clock_mhz reset_cycles interface_mode} {
                dict set cfgDict sensor $k [wb::getCfgField sensor.$k]
            }
            foreach k {suite waves coverage checker_mode} {
                dict set cfgDict processing $k [wb::getCfgField processing.$k]
            }
        }
    }
...
proc wb::validateConfig {} {
    ...
    foreach name {general.case_name io.output_dir} {
        if {[string trim [wb::getCfgField $name]] eq ""} {
            wb::log "ERROR: campo obbligatorio mancante: $name"
            set ok 0
        }
    }

    if {$ok} {
        wb::log "OK: configurazione valida"
    }
    return $ok
}
...
proc wb::openConfig {} {
    ...
    set f [tk_getOpenFile \
        -title "Apri configurazione JSON" \
    ...
    if {[catch {
        ...
        tk_messageBox -icon error -title "Errore apertura" \
            -message "Impossibile aprire il file JSON:\n$err"
        wb::log "ERROR: apertura config fallita: $err"
        return
    }

    wm title . "Workbench - [file tail $currentFile]"
    wb::log "OK: config caricata da $currentFile"
}

proc wb::saveConfigAs {} {
    variable currentFile
    set f [tk_getSaveFile -title "Salva configurazione" -defaultextension ".json" \
        -filetypes {{"JSON files" {.json}} {"All files" {*}}}]
    if {$f eq ""} { return }
    set currentFile $f
    wb::saveConfig
...
proc wb::saveConfig {} {
    ...
    if {[catch {
        ...
        wb::log "ERROR: salvataggio fallito: $err"
        return
    }

    wm title . "Workbench - [file tail $currentFile]"
    wb::log "OK: config salvata: $currentFile"
}
...
proc wb::useConfigInLauncher {} {
    ...
    if {$currentFile eq ""} {
        set ans [tk_messageBox -icon question -type yesno -title "Salvare prima" \
            -message "La configurazione non è ancora salvata. Vuoi salvarla adesso?"]
        if {$ans eq "yes"} {
            wb::saveConfig
        }
    }
    ...
    if {$currentFile eq ""} {
        wb::log "WARN: uso nel launcher annullato: nessun file config disponibile"
        return
    }
    ...
    wb::log "OK: config associata al launcher"
}
...
proc wb::buildLaunchCommand {} {
    ...
    switch -- $type {
        python_script {
            ...
            if {[string trim $launchInputData] ne ""} {
                lappend cmd --input $launchInputData
            }
...
proc wb::startRun {} {
    ...
    if {[catch {
        ...
        wb::log "ERROR: avvio processo fallito: $err"
        return
    }

    cd $oldDir

    wb::log "OK: processo avviato"
    wb::log "CMD: [join $cmd { }]"
    wb::log "Output dir: $outdir"
...
proc wb::onProcessReadable {chan} {
    ...
    if {[eof $chan]} {
        ...
        if {$closeMsg ne ""} {
            wb::log "WARN: processo terminato: $closeMsg"
        } else {
            wb::log "OK: processo terminato correttamente"
        }
        set running 0
        set processChan ""
        set processPid ""
...
proc wb::stopRun {} {
    ...
    if {$processPid ne ""} {
        ...
        wb::log "WARN: segnale di stop inviato al PID=$processPid"
    }
...
proc wb::openOutputDir {} {
    ...
    wb::log "OK: apertura output dir: $lastRunOutputDir"
}
    ...
    switch -- $currentDomain {
        radar {
            set scenarioRows {
                ...
                {fs_hz "Sampling freq [Hz]"}
                {target_count "Target count"}
                {environment "Environment"}
            }
            set sensorRows {
                {fc_hz "Carrier freq [Hz]"}
            ...
            set procRows {
                ...
                {integration_pulses "Integration pulses"}
                {tracker "Tracker"}
            }
        }
        satellite {
            ...
            set sensorRows {
                ...
                {gain_db "Gain [dB]"}
                {pointing_mode "Pointing mode"}
            }
            set procRows {
                {coverage_mode "Coverage mode"}
                {export_access_windows "Export access windows"}
                {estimator "Estimator"}
            }
        }
        underwater {
            ...
            set sensorRows {
                {center_freq_hz "Center freq [Hz]"}
                {array_elements "Array elements"}
                {gain_db "Gain [dB]"}
                {platform_speed_ms "Platform speed [m/s]"}
            }
            set procRows {
...
proc wb::buildUI {} {
    ...
    ttk::button .toolbar.saveas  -text "Salva come" -command wb::saveConfigAs
    ...
    pack .toolbar.new .toolbar.open .toolbar.save .toolbar.saveas \
         .toolbar.valid .toolbar.use .toolbar.run .toolbar.stop .toolbar.out \
         -side left -padx 4
    ...
    ttk::combobox .configTab.top.domain -state readonly \
        -values {radar satellite underwater hdl} \
        -textvariable wb::currentDomain
    ttk::button .configTab.top.preset -text "Carica preset" -command {
        wb::loadPreset $wb::currentDomain
    }
    ...
    bind .configTab.top.domain <<ComboboxSelected>> {
        wb::loadPreset $wb::currentDomain
    }
    ...
    ttk::combobox .launcherTab.box.c1 -state readonly \
        -values [wb::profileNames] \
        -textvariable wb::launchProfile
    ...
    # Log tab
    ttk::labelframe .logTab.box -text "Log runtime" -padding 8
    pack .logTab.box -fill both -expand 1

    set ui(log) [text .logTab.box.txt -wrap none]
    ttk::scrollbar .logTab.box.vsb -orient vertical -command "$ui(log) yview"
    ttk::scrollbar .logTab.box.hsb -orient horizontal -command "$ui(log) xview"
    $ui(log) configure -yscrollcommand ".logTab.box.vsb set" -xscrollcommand ".logTab.box.hsb set"

    grid $ui(log) -row 0 -column 0 -sticky nsew
    grid .logTab.box.vsb -row 0 -column 1 -sticky ns
    grid .logTab.box.hsb -row 1 -column 0 -sticky ew
    grid rowconfigure .logTab.box 0 -weight 1
    grid columnconfigure .logTab.box 0 -weight 1
    ...
    bind .launcherTab.box.c1 <<ComboboxSelected>> {
        wb::saveAppState
        wb::updateCommandPreview
    }
}
...
wb::updateCommandPreview
wb::log "OK: Workbench V2 inizializzato"
wb::log "OK: import JSON reale attivo"
wb::log "OK: log colorati attivi"