...
proc wf::loadSerialized {data} {
    ...
    foreach n [dict get $data nodes] {
        ...
        foreach key {launcherProfile configPath logPath workDir extraArgs notes} {
...
proc wf::highlightRunningNode {nodeId} {
    variable nodes
    variable canvas
...
proc wf::unhighlightNode {nodeId} {
    variable nodes
    variable canvas
    if {![info exists nodes($nodeId)]} { return }
    set rectId [dict get $nodes($nodeId) rect]
    if {$::wf::selectedNode eq $nodeId} {
        $canvas itemconfigure $rectId -outline blue -width 3
    } else {
        $canvas itemconfigure $rectId -outline black -width 2
    }
...
proc wf::executeNode {nodeId} {
    ...
    switch -- $type {
        Config {
            ...
            if {$path eq ""} {
                wf::log "Nodo Config senza configPath: $name" warn
                return
            }
            wf::openPath $path
        ...
        Log {
            ...
            if {$path eq ""} {
                wf::log "Nodo Log senza logPath: $name" warn
                return
            }
            wf::openPath $path
        ...
        Output {
            set outdir [dict get $nodes($nodeId) lastOutputDir]
            if {$outdir eq ""} {
                wf::log "Nodo Output senza lastOutputDir: $name" warn
                return
            }
            wf::openPath $outdir
            wf::log "Aperta cartella output del nodo '$name': $outdir" ok
            return
        }
        Process {
            ...
            cd $oldDir
            wf::highlightRunningNode $nodeId
...
proc wf::onProcessReadable {chan} {
    ...
    if {[eof $chan]} {
        ...
        if {$runningNode ne "" && [info exists nodes($runningNode)]} {
            ...
            wf::unhighlightNode $runningNode
        }
        ...
        if {[llength $::wf::executionQueue] > 0} {
...
proc wf::runWorkflowSequence {} {
    ...
    set queue {}
    foreach nodeId $ordered {
        if {[dict get $::wf::nodes($nodeId) type] eq "Process"} {
...
proc wf::buildUI {} {
    ...
    pack .toolbar.select .toolbar.add .toolbar.connect .toolbar.delete \
         .toolbar.sim .toolbar.runone .toolbar.runall .toolbar.stop .toolbar.save \
         -side left -padx 3
...
wf::buildUI
wf::loadRadarPreset
wf::log "Workflow Editor V2 operativo inizializzato" ok
